...
@Component({
  selector: 'app-fashion-style-client',
  standalone: false,
  templateUrl: './fashion-style-client.component.html',
  styleUrl: './fashion-style-client.component.css'
})
export class FashionStyleClientComponent {
  ...
  detailFashion(fashionId: string) {
    this._router.navigate(['fashion-detail-client', fashionId])
  }
  ...
  fashionsByStyle(style: string) {
    if (style === 'All Style') {
      ...
      this._router.navigate(['fashion-client']); // Use an empty string or replace with your home page route
    } else if (style !== '') {
      // If a specific style is selected, navigate to the fashions by style page
      this._router.navigate(['fashion-style-client', style]);
    }
  }
}
